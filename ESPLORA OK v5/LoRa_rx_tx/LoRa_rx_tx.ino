...
void loop() {
  ...
  if ((PAUSE && tx_legal && millis() - last_tx > (PAUSE * 100)) || button.isSingleClick()) {
    ...
    int sensorluz = analogRead(3)/4095*100; // Leer valor del primer sensor (Ejemplo)
    int sensorsuelo= analogRead(5)/4095*100; // Leer valor del segundo sensor

    String sensorData = String(t) + "," + String(sensorluz) + "," + String(h) + "," + String(sensorsuelo);
